...
void Sha256::update(const uint8_t* data, size_t len) {
  ...
  if (off < len) {
    std::memcpy(buffer_, data + off, len - off);
    buflen_ = len - off;
  }
}
...
std::array<uint8_t, 32> Sha256::digest() {
  ...
  size_t padlen = 0;
  pad[padlen++] = 0x80;
  while (((buflen_ + padlen) % 64) != 56) pad[padlen++] = 0;
  for (int i = 7; i >= 0; --i)
    pad[padlen++] = static_cast<uint8_t>((bitlen_ >> (i * 8)) & 0xff);
  update(pad, padlen);
  std::array<uint8_t, 32> out{};
  for (int i = 0; i < 8; ++i) {
    out[static_cast<size_t>(i*4)]   = static_cast<uint8_t>((state_[i] >> 24) & 0xff);
    out[static_cast<size_t>(i*4+1)] = static_cast<uint8_t>((state_[i] >> 16) & 0xff);
    out[static_cast<size_t>(i*4+2)] = static_cast<uint8_t>((state_[i] >> 8) & 0xff);
    out[static_cast<size_t>(i*4+3)] = static_cast<uint8_t>(state_[i] & 0xff);
  }
  return out;
}
